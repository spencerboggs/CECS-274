  ...
  while option != '0':
    ...
    if option == "1":
      ...
      while True:
        ...
        if input("Enter another variable? Y/N: ") != "Y":
          break
    elif option == "3":
      expression = input("Introduce the mathematical expression: ")
      if calculator.matched_expression(expression):
...
def menu_bookstore_system():
  ...
  while option != '0':
    ...
    if option == "r":
      ...
      p = input('Introduce the prefix: ')
      try:
        print(f'Added first matched title: {bookStore.addBookByPrefix(p)}')
      except ValueError:
...
def menu_palindrome_test():
    ...
    string = str(input("Enter a word/phrase: "))
    for i in string:
      palindrome.add(0, i)
